...
## Edge Research Lab (intransitivita, kalibrace, CLV)

Pro výzkum value-bet hypotéz je v repu nový skript `edge_research_lab.py`:

### 1) Intransitivity backtest

```bash
python3 edge_research_lab.py intransitivity-backtest --csv-files sample_atp_matches.csv --surface Hard
```

Výstup ukáže baseline upset rate vs upset rate ve "vysoké intransitivitě" (A>B, B>C, C>A evidence přes historii).

### 2) Calibration report

```bash
python3 edge_research_lab.py calibration-report --csv-files sample_atp_matches.csv --surface Hard
```

Výstup reportuje Brier/ECE a vybere slope s nejlepší train Brier (kalibrace > accuracy).

### 3) CLV report

```bash
python3 edge_research_lab.py clv-report --bets-csv sample_bets_clv.csv
```

Pro vlastní tracking používej CSV se sloupci:
`timestamp,market,taken_odds,close_odds,stake,pnl`

KPI:
- Avg CLV (%),
- ROI (%),
- rozpad podle marketu.
